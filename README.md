...
昨日收盘价（隐藏）：125根K线缓存最新一根的收盘价（前复权），盘中为上一交易日收盘、收盘后与现价一致。
现价（隐藏）：盘中实时价格或收盘价。
涨跌幅（隐藏）：现价相对昨日收盘价的涨跌幅，来自行情接口，判断涨跌。
股息率：上一个财年的派息/现价，作为股东回报参考。
  默认最低3%股息率，可在页面调整。
息增年：派息持续增加的年份，0年即上一个财年的派息额小于上上年。
FCF（隐藏）：窄口径自由现金流。
FCF/股息：窄口径FCF/股息，评估自由现金流对股息的覆盖率。
FCF/现价：窄口径FCF/现价，评估赚钱能力。
...
反弹：现价相对于最近20个交易日的最低价的涨幅，评估月内波动。
回落：现价相对于最近20个交易日的最高价的跌幅，评估月内波动。
派息历史（隐藏）：分红配送相关的公告变化。

### 数据来源与抓取方式
数据按需抓取：只有缓存过期的数据才会重新请求，一次页面刷新会按优先级先抓屏蔽相关数据（行业、收益、息增年），被屏蔽的股票不再请求其余数据。

腾讯（批量行情接口，每次100只，约0.4秒一次）：
- 行情：qt.gtimg.cn，每批100只返回现价、昨收、开盘、最高、最低、成交量、成交额、涨跌幅。
  更新频率：与前端自动刷新同步，盘中每5分钟更新一次，收盘后只更新一次（当日收盘值），
  周末、盘前不更新；未打开前端时无任何请求。

腾讯（批量接口，每次100只）：
- 市值：qt.gtimg.cn 的field 45（流通市值），单位亿，
  更新频率：周五收盘后刷新，周一至周四保持上周五数据不刷新。

腾讯（K线接口，逐股请求，每股一次，前复权日线）：
- MA120、反弹、回落：web.ifzq.gtimg.cn，一次请求取125根日K，同份数据复用：
  MA120取后120根收盘价，反弹取后25根盘中最低价，回落取后25根盘中最高价。
  125根 = 120根MA120 + 盘中排除当日未完成K线1根 + 少量容错余量。
  更新频率：每个交易日收盘后最多一次（页面打开时触发），盘中、周末、假日零请求；
  首次无缓存、停机补数、除息日调整时重新请求。
- K线原始数据缓存于 cn_high_dividend_kline_cache，最多125根/股，多余删除：
  首次无缓存时请求125根；缓存缺最新交易日K线（如容器停机未更新）时重新请求覆盖；
  缓存已含最新交易日（盘中为最近交易日、收盘后为当天）时不再请求外部接口，直接用缓存重算三个指标；
  除息日前后复权价整体调整，重新请求覆盖125根。
  周末、假日打开页面不再重复请求K线。
- 昨日收盘价（隐藏）：K线缓存最新一根的收盘价（前复权），与MA120同口径，
  盘中为上一交易日收盘，收盘后为当天收盘（与现价一致）。

东财（批量接口，约1秒一次）：
- 派息、派息历史：datacenter RPT_SHAREBONUS_DET，历年分红配送记录，每批10只约320批。
  更新频率：交易日每天8点后检查一次，16点至23点最多每4小时复查一次。
- 扣非、收益：datacenter RPT_F10_FINANCE_MAINFINADATA，最新季报扣非净利润同比、上年年报稀释每股收益，同一请求返回，每批20只约160批。
  更新频率：交易日每天8点后检查一次，16点至23点最多每4小时复查一次。
- FCF、FCF/股息、FCF/现价：datacenter RPT_DMSK_FN_CASHFLOW，年报经营现金流净额与购建资产支出，据此计算窄口径FCF，每批20只约160批。
  更新频率：年报季（1-5月）交易日每天8点后检查一次、16点至23点复查一次；非年报季最多7天一次。

东财（批量接口，每次100只）：
- 行业：datacenter RPT_F10_ORG_BASICINFO，申万二级行业。
  更新频率：只在无缓存时请求一次，行业基本不变不需要刷新。
